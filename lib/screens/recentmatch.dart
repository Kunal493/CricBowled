...
const String baseUrl = 'https://cricbuzz-cricket.p.rapidapi.com/matches/v1/recent';
...
class CricketMatch {
  ...
  final Uint8List? team2LogoBytes;
  ...
  factory CricketMatch.fromJson(Map<String, dynamic> json) {
    ...
    return CricketMatch(
      ...
      status: matchInfo['status'] as String,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var typeMatch in data["typeMatches"]) {
          // Check if "seriesMatches" exist and is not empty
          if (typeMatch.containsKey("seriesMatches") && typeMatch["seriesMatches"].isNotEmpty) {
            for (var seriesMatch in typeMatch["seriesMatches"]) {
              // Check if "seriesAdWrapper" exists
              if (seriesMatch.containsKey("seriesAdWrapper")) {
                var matches = seriesMatch["seriesAdWrapper"]["matches"] ?? []; // Get matches list or empty list
                if (matches.isNotEmpty) {
                  filteredData[counter] = matches[0]["matchInfo"] ?? {}; // Use counter as key, handle missing "matchInfo"
                  counter++;
                }
              }
            }
